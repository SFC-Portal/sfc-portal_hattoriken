...
echo ""
echo "  1. 環境変数を設定:"
echo "     - frontend/.env.local  （frontend/.env.local.example をコピーして編集）"
echo "     - backend/.env         （backend/.env.example をコピーして編集）"
echo ""
echo "  2. 開発サーバーを起動:"
echo "     フロントエンド: cd frontend && npm run dev"
echo ""
echo "     バックエンド（毎回この順番で実行）:"
echo "       cd backend"
echo "       source venv/bin/activate   ← 必須: 仮想環境を有効化"
echo "       uvicorn app.main:app --reload"
echo ""
echo "     ※ ターミナルを開き直すたびに source venv/bin/activate が必要です"
echo "     ※ プロンプトに (venv) が表示されていれば有効化済みです"
echo ""
